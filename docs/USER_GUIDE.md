...
2. **ai_analysis.md**：模型产出的阅读友好 Markdown——
   ...
   - 一级标题 `[归属组]文件昵称_实际文件名`；
   - 正文固定四节：**一、结论（事实清单）**（分点：文本/对照规则（列数+字段名）/条数偏离值/差异栏位分布占比/尾部条数核对）→ **二、主键唯一定位检测**（主键无法唯一定位时红色 🔴 突出提示配置需重检）→ **三、差异栏位分析**（逐栏位：特征事实 + **TOP20 差异明细表**（主键 + A/B 值）+ 克制观察，不做业务臆测）→ **四、文件尾部对比**（纯表格事实）；
   - 文末附栏位属性表。
